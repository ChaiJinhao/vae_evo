...
def PEPG(obj, pop_dim, popsize=512, num_steps=10, sigma_init=1, elite_ratio=0.1, lower_bound=-4,
                 upper_bound=4):
    es = PEPG_model(
        num_params=pop_dim,
        ...
        elite_ratio=elite_ratio  # Elite ratio can lead to multiple solutions
    )
    ...
    return fitnesses, populations, es, np.stack(mus), np.stack(sigmas)